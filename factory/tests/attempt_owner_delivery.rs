...
impl World {
    fn new() -> Self {
        ...
        let run = Run::new(
            ...
            "controlled owner transport",
            "factory-owner-transport-test",
        )
        .unwrap();
    ...
    fn action(&self, operation: FactoryAttemptOperation) -> Output {
        let request = FactoryAttemptActionRequest {
            ...
            projection_ref: "projection:transport".into(),
            caller: caller(),
    ...
    fn start(&self, protected: bool) {
        ...
        let demand = ExecutionDemand {
            ...
            agency_ref: Some("agency:transport".into()),
            profile_ref: None,
        ...
        let disposition = SituatedExecutionDisposition {
            ...
            placement: protected.then(|| PlacementProtection {
                now_ref: "now:test".into(),
                now_path: self.dir.path().display().to_string(),
                policy_ref: "policy:test".into(),
                policy_revision: "policy-r1".into(),
                authority_ref: "authority:placement".into(),
                writable_paths: BTreeSet::new(),
                protected_paths: BTreeSet::from(["source:test".into()]),
                required_coverage: BTreeSet::from(["source-protection".into()]),
                effective_coverage: BTreeSet::from(["source-protection".into()]),
                write_boundary_ref: Some("declared-not-enforced".into()),
                material_receipt_ref: None,
            }),
    ...
    fn request(&self, id: &str, action: &str) -> Value {
        let packet = if action == "send" {
            ...
            json!({"action":"delivery", "agent_session":SESSION, "delivery_ref":"delivery:test"})
        };
    ...
    fn owner(&self, request: &Value) -> Output {
        self.call("owner-action", Some(request))
    }
...
fn caller() -> FactoryActionCaller {
    FactoryActionCaller {
        caller_ref: "agent:transport-test".into(),
        projection_kind: FactoryActionProjectionKind::Headless,
        lineage: vec![
            "agency:transport-test".into(),
            "agent:transport-test".into(),
        ],
    }
}
fn authority() -> ProjectedFactoryActionAuthority {
    ProjectedFactoryActionAuthority {
        authority_ref: "authority:transport-test".into(),
...
fn dispatch_persists_intent_and_passes_exact_bounds_before_real_subprocess_call() {
    ...
    let text = calls[0]
        ...
        .unwrap();
    assert!(
        text.contains("source-revision-7")
            && text.contains("947ce7a")
            && text.contains("cite exact files and revisions")
    );
    let reading = world.reading();
    assert_eq!(
        reading.attempts[0].execution_ref.as_deref(),
        Some(EXECUTION)
    );
...
fn lost_response_recovers_original_intent_without_replaying_task() {
    ...
    assert!(!world
        .action(FactoryAttemptOperation::Fail {
            attempt_ref: ATTEMPT.into(),
            reason: "unknown effects".into(),
            evidence_refs: BTreeSet::from(["evidence:unknown".into()])
        })
        .status
        .success());
    world.mode("returned");
    ...
    success(world.action(FactoryAttemptOperation::Fail {
        attempt_ref: ATTEMPT.into(),
        reason: "explicit task failure despite completed provider turn".into(),
    ...
    assert_eq!(
        world
            .calls()
            .iter()
            .filter(|request| request["action"] == "send")
            .count(),
        1
    );
...
fn declared_coverage_is_not_accepted_as_enforcement_on_plain_session_path() {
    ...
    assert!(
        !output.status.success(),
        "declared placement was incorrectly promoted to enforced protection"
    );
    assert!(world.calls().is_empty());
    assert_eq!(before, fs::read(world.state()).unwrap());
}
...
fn development_alias_uses_same_canonical_owner_action_and_readback() {
    ...
    assert_eq!(
        world.reading().attempts[0].execution_ref.as_deref(),
        Some(EXECUTION)
    );
}
