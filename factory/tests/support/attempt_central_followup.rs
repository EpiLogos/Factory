...
fn two_preparations_on_the_same_opening_revision_have_one_winner() {
    ...
    let args = ["attempt".into(), "prepare".into(), world.state(), "-".into(), "--json".into()];
    let one = spawn(&args, Some(&first));
    let two = spawn(&args, Some(&second));
    let outputs = [one.wait_with_output().unwrap(), two.wait_with_output().unwrap()];
...
fn native_central_return_keeps_the_allocated_now_without_including_the_document() {
    let binary = std::env::var_os("FACTORY_TEST_CTRL").expect("native ctrl is required, no protocol double");
    let binary = std::path::PathBuf::from(binary);
    let world = World::new();
    world.start(false);
    setup(&world, false);
    let agent_ref = world.reading().attempts[0].disposition.participant.agent_ref.clone();
    ...
    let relations_path = world.dir.path().join("Control/relations/source-relations.json");
    ...
    let invoke = |action: &str, input: Value, token: &str| {
        let output = Command::new(&binary)
            .args(["--json", "--root"]).arg(world.dir.path())
            .args(["action", "run", action]).arg(input.to_string())
            .env("CENTRAL_NATIVE_TOKEN", token).output().unwrap();
        let response = success(output);
        assert_eq!(response["ok"], true, "{response}");
        response["data"].clone()
    };
    let policy = invoke("central.work.policy", json!({}), "");
    let document = invoke("central.document.create", json!({
    ...
    let document_path = world.dir.path().join(document["source"]["path"].as_str().unwrap());
    ...
    let now_ref = &prepared["observation"]["payload"]["detail"]["checkpoint"]["allocation"]["now_ref"];
    ...
    success(world.action(FactoryAttemptOperation::RecordVerification {
        ...
        verification: VerificationReceipt {
            ...
            owner_ref: "factory".into(), source_revision: world.workflow.source.revision.clone(),
            outcome: VerificationOutcome::Passed, obligations: unit.verification_obligations.clone(), evidence_refs: evidence.clone(),
        },
    }));
    success(world.action(FactoryAttemptOperation::ReturnArtifact {
        attempt_ref: ATTEMPT.into(),
        artifact: epilogos_factory::orchestration::ReturnedArtifact {
            artifact_ref: "artifact:native-join-controlled".into(), subject_ref: unit.subject_ref.to_string(),
            subject_revision: unit.basis_revision.clone(), producing_execution_ref: EXECUTION.into(),
            evidence_refs: evidence.clone(), semantic_difference: "Controlled source report for native receiving".into(),
        },
        readable_return: ReadableReturn {
            return_ref: "return:native-join-controlled".into(), summary: "Controlled native Return, not human inclusion".into(),
            artifact_refs: BTreeSet::from(["artifact:native-join-controlled".into()]), evidence_refs: evidence,
            receiving_ref: None, receiving_source_revision: None, archive_refs: BTreeSet::new(), regression_observation_refs: BTreeSet::new(),
        },
    }));
    let request = json!({"contract":"factory.attempt-receiving-action/v1","requestRef":"receiving:native-join","projectionRef":"projection:native-join",
        ...
        "target":{"sourceRef":document["source"]["ref"],"documentId":document["document_id"],"expectedSourceRevision":document["revision"]["revision"]}});
    let mut child = Command::new(env!("CARGO_BIN_EXE_factory"))
        .args(["attempt", "receiving"]).arg(world.state()).args(["-", "--json"])
        .env("CENTRAL_NATIVE_TOKEN", "factory-native-central-agent-fixture")
    ...
    assert_eq!(actual["record"]["task_ref"], world.reading().attempts[0].task_ref);
    ...
    assert_eq!(world.reading().attempts[0].readable_return.as_ref().unwrap().receiving_ref.as_deref(), actual["return_ref"].as_str());
}
