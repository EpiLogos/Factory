...
fn caller() -> FactoryActionCaller {
    FactoryActionCaller {
        caller_ref: "agent:material-test".into(),
...
fn authority() -> ProjectedFactoryActionAuthority {
    ProjectedFactoryActionAuthority {
        authority_ref: "authority:material-test".into(),
...
"#,
    ...
    fn receipt(&self) -> PathBuf {
        self.dir.path().join("world.json")
    }
    fn owner(&self) -> PathBuf {
        self.dir.path().join("owner.py")
    }
    ...
    fn action(&self, operation: FactoryAttemptOperation) -> Output {
        let request = FactoryAttemptActionRequest {
            ...
            projection_ref: "projection:material-tests".into(),
            caller: caller(),
            run_ref: RUN.parse().unwrap(),
            expected_revision: self.read().revision,
            authority: authority(),
            operation,
        };
        binary(
    ...
    fn disposition(&self, key: &str) -> SituatedExecutionDisposition {
        ...
        let selection = accept_aikit_selection(
            ExecutionDemand {
                ...
                agency_ref: Some("agency:material-test".into()),
                profile_ref: None,
        ...
        SituatedExecutionDisposition {
            ...
            body: ExecutionBody {
                ...
                agent_session_ref: format!("session:{key}"),
                session_space_ref: "space:test".into(),
                material_world_ref: Some(MATERIAL.into()),
                workcell_ref: Some(WORKCELL.into()),
            },
            placement: None,
            ...
            budget: ExecutionBudget {
                cost_ceiling_usd: None,
                latency_preference_ms: None,
                wall_clock_timeout_ms: Some(self.timeout),
    ...
    fn start(&self, key: &str, attempt: &str) -> Output {
        self.action(FactoryAttemptOperation::StartSerial {
    ...
    fn request(
        ...
        FactoryAttemptOwnerRequest {
            ...
            projection_ref: "projection:material".into(),
            caller: caller(),
            run_ref: RUN.parse().unwrap(),
            expected_revision: self.read().revision,
            authority: authority(),
            attempt_ref: ATTEMPT.into(),
            execution_ref: format!("factory-attempt:{ATTEMPT}"),
...
fn native_material_reads_preserve_observations_without_creating_execution_or_return() {
    ...
    for (index, operation) in [
        ...
        WorkcellWorldOperation::Observe,
        WorkcellWorldOperation::Collect,
        WorkcellWorldOperation::Expose,
    ]
    .into_iter()
    .enumerate()
    {
        ...
        assert_eq!(
            response["transportObservation"]["payload"]["detail"]["validated"],
            true
        );
        assert_eq!(
            response["transportObservation"]["payload"]["detail"]["taskReturnEstablished"],
            false
        );
    }
...
fn exact_material_replay_is_byte_stable_and_changed_requests_are_refused() {
    ...
    changed.expected_revision = world.read().revision;
...
fn foreign_material_response_is_retained_as_uncertain_and_blocks_new_effects() {
    ...
    world.mode("foreign");
    let response =
        success(world.invoke(&world.request("foreign", WorkcellWorldOperation::Observe)));
    assert_eq!(response["needsReconciliation"], true);
    assert_eq!(
        response["ownerReceipt"]["payload"]["world_ref"],
        "world:foreign"
    );
    assert_eq!(
        response["transportObservation"]["payload"]["detail"]["validated"],
    ...
    refused(
        world.invoke(&world.request("recover:after-foreign", WorkcellWorldOperation::Recover)),
        "unresolved material",
    );
    assert_eq!(world.calls(), 1);
}
...
fn material_recovery_retains_predecessor_and_requires_new_arrangement_not_body_rewriting() {
    let world = World::new();
    let response =
        success(world.invoke(&world.request("recover:one", WorkcellWorldOperation::Recover)));
    assert_eq!(response["needsReconciliation"], false);
    assert_eq!(
        response["ownerReceipt"]["payload"]["previous_world_ref"],
        MATERIAL
    );
    assert_eq!(
        world.read().attempts[0]
            .disposition
            .body
            .material_world_ref
            .as_deref(),
    ...
    refused(
        world.invoke(&world.request("recover:old", WorkcellWorldOperation::Recover)),
        "superseded material",
    );
    refused(
        world.start("peer-read", "attempt:peer"),
        "material lifecycle",
    );
    fs::write(
        world.receipt(),
        serde_json::to_vec(&response["ownerReceipt"]["payload"]["world"]).unwrap(),
    )
    .unwrap();
    assert_eq!(
        success(world.invoke(&world.request("read:successor", WorkcellWorldOperation::Observe)))
            ["ownerReceipt"]["payload"]["world_ref"],
        "world:recovered-test"
    );
...
fn release_requires_quiescence_preserves_input_and_does_not_complete_task() {
    let world = World::new();
    refused(
        world.invoke(&world.request("release:early", WorkcellWorldOperation::Release)),
        "explicit quiescence",
    );
    ...
    let response =
        success(world.invoke(&world.request("release:ready", WorkcellWorldOperation::Release)));
...
fn release_refuses_other_active_material_users() {
    let world = World::new();
    success(world.start("peer-read", "attempt:peer"));
    world.quiesce();
    refused(
        world.invoke(&world.request("release:shared", WorkcellWorldOperation::Release)),
        "another active",
    );
    assert_eq!(world.calls(), 0);
}
...
fn release_intent_prevents_a_later_native_start_using_the_same_material() {
    ...
    assert!(
        world.dir.path().join("entered").exists(),
        "owner did not receive the durably reserved call"
    );
    refused(
        world.start("peer-read", "attempt:peer"),
        "material lifecycle",
    ...
    assert_eq!(
        success(handle.join().unwrap())["needsReconciliation"],
        false
    );
}
...
fn material_transport_deadline_is_finite_without_claiming_worker_quiescence() {
    ...
    let response =
        success(world.invoke(&world.request("read:timeout", WorkcellWorldOperation::Observe)));
...
fn changed_input_path_cannot_redirect_the_admitted_native_operation() {
    ...
    world.mode("swap");
    let response =
        success(world.invoke(&world.request("read:stable-copy", WorkcellWorldOperation::Inspect)));
    assert_eq!(response["needsReconciliation"], false);
...
fn stale_unauthorised_foreign_and_credential_bearing_calls_do_not_reach_transport() {
    ...
    refused(world.invoke(&request), "stale Factory");
    request.expected_revision = world.read().revision;
    request.authority.action_authorised = false;
    refused(world.invoke(&request), "authority");
    request.authority.action_authorised = true;
    if let NativeOwnerInvocation::WorkcellWorld { authorization, .. } = &mut request.invocation {
        *authorization = Some("test-private-never-retain".into());
    }
    let output = world.invoke(&request);
    assert!(!String::from_utf8_lossy(&output.stderr).contains("test-private-never-retain"));
...
fn material_alias_and_capability_discovery_use_the_same_native_contract() {
    ...
    for name in ["attempt.material", "development.attempt.material"] {
        assert!(capabilities["commands"]
            .as_array()
            .unwrap()
            .contains(&json!(name)));
    }
    assert!(capabilities["nativeContracts"]
        .as_array()
        .unwrap()
