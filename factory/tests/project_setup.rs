...
fn setup_is_idempotent_and_refuses_identity_collision_without_changing_state() {
    ...
    assert_eq!(setup(root.path(), "control:root", None).unwrap()["status"], "already-present");
    assert_eq!(locate(root.path()).unwrap()["projectRef"], first["projectRef"]);
    assert!(setup(root.path(), "different-project", None).is_err());
    assert_eq!(fs::read(path).unwrap(), bytes);
    assert_eq!(fs::read(root.path().join(".factory/project.json")).unwrap(), metadata);
}
...
fn relocated_project_retains_original_link_and_refuses_changed_source() {
    ...
    fs::write(&source, r#"{"schema":"central.project/v1","project_id":"product"}"#).unwrap();
    ...
    assert_eq!(state["state"]["centralProjectLinkRelocations"][0]["previous"]["sourcePath"], original_source.to_str().unwrap());
    assert_eq!(state["state"]["centralProjectLinkRelocations"][0]["current"]["sourcePath"], relocated_source.canonicalize().unwrap().to_str().unwrap());
    setup(&new, "product", Some(&relocated_source)).unwrap();
    assert_eq!(fs::read(path).unwrap(), bytes);
    fs::write(&relocated_source, r#"{"schema":"central.project/v1","project_id":"different"}"#).unwrap();
...
fn public_cli_setup_and_locate_return_native_project() {
    let root = tempfile::tempdir().unwrap();
    let run = |args:&[&str]| std::process::Command::new(env!("CARGO_BIN_EXE_factory")).args(args).output().unwrap();
    ...
    assert!(!run(&["project", "setup", path, "foreign", "--json"]).status.success());
...
fn typed_first_commission_and_replay_keep_initialized_identity_and_source() {
    ...
    fs::write(&request, include_str!("../workflow-sdk/examples/commission.json")).unwrap();
    fs::write(&source, include_str!("../workflow-sdk/examples/single.workflow.ts")).unwrap();
    ...
    let commission = || {
        let output = std::process::Command::new(env!("CARGO_BIN_EXE_factory"))
            .args(["workflow", "commission", state_path, request.to_str().unwrap(), source.to_str().unwrap(), "--json"])
            .output().unwrap();
        assert!(output.status.success(), "{}", String::from_utf8_lossy(&output.stderr));
        serde_json::from_slice::<serde_json::Value>(&output.stdout).unwrap()
    };
    let receipt = commission();
    assert_eq!(receipt["contract"], "factory.workflow-commission-receipt/v1");
    assert_eq!(receipt["execution"], "not-requested");
    let bytes = fs::read(state_path).unwrap();
    let provider = epilogos_factory::developmental_read::FactoryDevelopmentalFileProvider::open(state_path).unwrap();
    let state = provider.state();
    assert_eq!(state.build.project().reference().to_string(), initialized["projectRef"]);
...
fn native_central_source_is_verified_and_existing_files_survive_setup() {
    ...
    fs::write(root.path().join(".factory/authored.workflow.ts"), "preserve authored work").unwrap();
    let initialized = setup(root.path(), "product", Some(&source)).unwrap();
    let state: serde_json::Value = serde_json::from_slice(&fs::read(initialized["statePath"].as_str().unwrap()).unwrap()).unwrap();
    assert_eq!(state["state"]["journeys"].as_array().unwrap().len(), 0);
    let links = state["state"]["centralProjectLinks"].as_object().unwrap();
    assert_eq!(links.values().next().unwrap()["centralProjectRef"], "product");
    assert_eq!(fs::read_to_string(root.path().join(".factory/authored.workflow.ts")).unwrap(), "preserve authored work");
}
...
fn central_names_with_spaces_keep_their_identity_and_get_unambiguous_factory_keys() {
    ...
    for (index, name) in ["My Project", "My%20Project", "日本語"].iter().enumerate() {
        ...
        fs::write(&source, serde_json::json!({"schema":"central.project/v1","project_id":name}).to_string()).unwrap();
...
fn concurrent_setup_converges_and_incomplete_placement_recovers() {
    ...
    let handles: Vec<_> = (0..2).map(|_| {
        ...
        std::thread::spawn(move || { barrier.wait(); setup(&root, "same", None).unwrap() })
    }).collect();
    let results: Vec<_> = handles.into_iter().map(|h| h.join().unwrap()).collect();
    assert_eq!(results.iter().filter(|v| v["status"] == "created").count(), 1);
    let state = results[0]["statePath"].as_str().unwrap();
    let bytes = fs::read(state).unwrap();
    fs::remove_file(root.path().join(".factory/project.json")).unwrap();
    assert_eq!(setup(root.path(), "same", None).unwrap()["status"], "already-present");
