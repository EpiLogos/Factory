...
fn call(
    ...
    let value: Value = serde_json::from_slice(&output.stdout)
        .map_err(|error| format!("Central did not return a readable ActionResult: {error}"))?;
...
pub fn execute(path: &Path, request: ReceivingRequest) -> Result<Value, String> {
    ...
    if let Some(previous) = &previous {
        ...
        return Err("no retained receiving request to recover".into());
    }
    if reading.revision != request.expected_revision {
        return Err("stale Factory revision before receiving call".into());
    }
    let known_receipt = attempt
        .readable_return
        .as_ref()
        .and_then(|returned| returned.receiving_ref.as_ref())
        .cloned()
        .or_else(|| {
            previous
                ...
                .and_then(|receipt| {
                    receipt
                        .payload
                        .pointer("/detail/centralResponse/data/return_ref")
                })
                .and_then(Value::as_str)
                .map(str::to_owned)
        });
    let (operation, call_input) = if request.recover {
        if let Some(reference) = known_receipt {
            let mut read = json!({"return_ref":reference});
            if let Some(project) = &request.central.project {
                read["project"] = json!(project);
            }
            ("central.receiving.read", read)
        } else {
            ("central.receiving.submit", input.clone())
        }
    } else {
        ("central.receiving.submit", input.clone())
    };
    ...
    let response = match call(&request.central, operation, &call_input) {
        ...
        Err(failure) => {
            let uncertain = stamp(
                ...
                json!({"failure":failure,"recovery":"explicit recover uses the same native producer key, never a replacement Return"}),
            );
