...
pub fn apply_attempt_action(
    ...
    {
        return Err(invalid("native Run changed during Action admission; reread"));
    }
...
fn validate_reading(reading: &FactoryAttemptReading) -> Result<(), FactoryAttemptError> {
    ...
    for record in &reading.attempts {
        ...
        let leg = reading.legs.get(&record.workflow_unit_ref)
            .ok_or_else(|| invalid("attempt record is not linked to a coordinator leg"))?;
        if !leg.attempts.iter().any(|attempt| attempt.execution_ref == execution) {
            return Err(invalid("attempt record is not linked to its execution history"));
        }
    }
    for leg in reading.legs.values() {
        for attempt in &leg.attempts {
            if !identities.contains(attempt.execution_ref.as_str()) {
                return Err(invalid("coordinator history has an untracked native attempt"));
            }
        }
    }
    Ok(())
...
fn validate_operation(
    ...
    match operation {
        StartSerial { disposition, retry_grant, tracking, parent_journey_ref, .. } => {
            validate_start(run, disposition, retry_grant.as_ref())?;
            nonempty(parent_journey_ref, "parent Journey")?;
            for fact in tracking { validate_fact(fact)?; }
        }
        StartFork { attempts, parent_journey_ref } => {
            nonempty(parent_journey_ref, "parent Journey")?;
            ...
            for start in attempts {
                ...
                for fact in &start.tracking { validate_fact(fact)?; }
            }
        }
        ...
        Fail { attempt_ref, evidence_refs, .. } => {
            let record = current_attempt(reading, attempt_ref)?;
            if has_uncertain_operation(record) {
                return Err(invalid("uncertain owner effects require reconciliation, not failure/writer release"));
            }
            if !evidence_refs.is_superset(&partial_effects(record)) {
                return Err(invalid("failure evidence must retain every known partial effect"));
            }
        }
        Retry { workflow_unit_ref, task_ref, parent_journey_ref, grant_ref,
            disposition, tracking, reresolution, .. } => {
            let prior = current_for_unit(reading, workflow_unit_ref)?;
            let leg = &reading.legs[workflow_unit_ref];
            if prior.task_ref != *task_ref || leg.delegation.parent_journey_ref != *parent_journey_ref {
            ...
            if prior.disposition.budget.retry_grant_ref.as_deref() != Some(grant_ref.as_str()) {
                return Err(invalid("retry cannot switch the previously consumed retry grant"));
            }
            ...
            if let Some(limit) = prior.disposition.budget.wall_clock_timeout_ms {
                if disposition.budget.wall_clock_timeout_ms.is_none_or(|next| next > limit) {
            ...
            if let Some(limit) = prior.disposition.budget.cost_ceiling_usd {
                if disposition.budget.cost_ceiling_usd.is_none_or(|next| next > limit) {
            ...
            match (&prior.disposition.placement, &disposition.placement) {
                (Some(before), Some(after)) => {
                    if !after.protected_paths.is_superset(&before.protected_paths)
                        || !after.required_coverage.is_superset(&before.required_coverage)
                    {
                        return Err(invalid("retry cannot drop protected sources or required coverage"));
                    }
                    if before.now_ref != after.now_ref
                        && reresolution.as_ref().and_then(|value| value.replacement_now_ref.as_deref())
                            != Some(after.now_ref.as_str())
                    {
                        return Err(invalid("changed NOW requires explicit matching re-resolution"));
                    }
                }
            ...
            if prior.disposition.body.material_world_ref != disposition.body.material_world_ref
                && reresolution.as_ref().and_then(|value| value.replacement_material_ref.as_deref())
                    != disposition.body.material_world_ref.as_deref()
            {
                return Err(invalid("changed material requires explicit matching re-resolution"));
            }
            if prior.disposition.body.harness_ref != disposition.body.harness_ref
                && reresolution.as_ref().and_then(|value| value.replacement_harness_ref.as_deref())
                    != Some(disposition.body.harness_ref.as_str())
            {
                return Err(invalid("changed harness requires explicit matching re-resolution"));
            }
            if !partial_effects(prior).is_empty() || prior.disposition != *disposition {
                let resolution = reresolution.as_ref()
                    .ok_or_else(|| invalid("partial effects or changed conditions require explicit re-resolution"))?;
                if resolution.source_revision != disposition.participant.source_revision
                    || !resolution.evidence_refs.is_superset(&partial_effects(prior))
                {
        ...
        BindDispatch { attempt_ref, execution_ref, receipt } => {
            let record = current_attempt(reading, attempt_ref)?;
            ...
            {
                return Err(invalid("session, delivery and execution identities are not interchangeable"));
            }
        }
        ReturnArtifact { attempt_ref, .. } => {
            let record = attempt(reading, attempt_ref)?;
            if has_uncertain_operation(record) {
                return Err(invalid("uncertain outcome cannot become a completed Return"));
            }
            let verification = record.verifications.last()
                .ok_or_else(|| invalid("Return needs current verification"))?;
            if verification.outcome != VerificationOutcome::Passed
                || !verification.obligations.is_superset(&record.disposition.verification_obligations)
            {
                return Err(invalid("a later failed/unknown verification supersedes an earlier pass"));
            }
            let dispatch = record.dispatch.as_ref()
                .ok_or_else(|| invalid("Return requires an actual owner dispatch"))?;
            let last = record.observations.iter().rev().find(|receipt| {
                receipt.owner_ref == dispatch.owner_ref && receipt.operation_ref == dispatch.operation_ref
            }).unwrap_or(dispatch);
            if last.phase != OwnerOperationPhase::Returned {
                return Err(invalid("submitted, failed or cancelled delivery is not completed provider work"));
            }
        }
        RecordTracking { fact, .. } => validate_fact(fact)?,
        RecordObservation { .. } | RecordVerification { .. } | RecordReresolution { .. }
        | AdvanceSubject { .. } | AttachReceiving { .. } | AttachArchive { .. } => {}
    }
    Ok(())
}
...
fn validate_start(
    ...
    if disposition.selection.demand.project_ref != run.project_ref().to_string() {
        return Err(invalid("Execution Intelligence demand names another Project"));
    }
    if disposition.budget.cost_ceiling_usd.is_some_and(|cost| !cost.is_finite() || cost < 0.0)
    ...
    if let Some(grant) = grant {
        if grant.grant_ref.trim().is_empty() || grant.attempts_allowed == 0 {
            return Err(invalid("retry grant must have an identity and a positive total bound"));
        }
    }
    Ok(())
}
...
fn attempt<'a>(reading: &'a FactoryAttemptReading, reference: &str) -> Result<&'a FactoryAttemptRecord, FactoryAttemptError> {
    reading.attempts.iter().find(|record| record.attempt_ref == reference)
        .ok_or_else(|| FactoryAttemptError::UnknownAttempt(reference.into()))
}

fn current_attempt<'a>(reading: &'a FactoryAttemptReading, reference: &str) -> Result<&'a FactoryAttemptRecord, FactoryAttemptError> {
    let record = attempt(reading, reference)?;
    let current = reading.legs.get(&record.workflow_unit_ref)
        .ok_or_else(|| invalid("attempt has no native leg"))?;
    if current.execution_ref != execution_identity(record) {
        return Err(invalid("historical attempt cannot mutate its replacement; history remains readable"));
    }
    Ok(record)
}

fn current_for_unit<'a>(reading: &'a FactoryAttemptReading, unit: &WorkflowUnitRef) -> Result<&'a FactoryAttemptRecord, FactoryAttemptError> {
    let leg = reading.legs.get(unit).ok_or_else(|| invalid("no prior leg for retry"))?;
    reading.attempts.iter().find(|record| {
...
fn partial_effects(record: &FactoryAttemptRecord) -> BTreeSet<String> {
    record.dispatch.iter().chain(record.observations.iter())
        .flat_map(|receipt| receipt.partial_effect_refs.iter().cloned()).collect()
}
...
fn has_uncertain_operation(record: &FactoryAttemptRecord) -> bool {
    ...
    for receipt in record.dispatch.iter().chain(record.observations.iter()) {
        ...
        match receipt.phase {
            OwnerOperationPhase::Uncertain | OwnerOperationPhase::Dispatching => { uncertain.insert(identity); }
            OwnerOperationPhase::ReconciledNoReplay => { uncertain.remove(&identity); }
            _ => {}
        }
    }
    !uncertain.is_empty()
...
fn validate_fact(fact: &AttemptTrackingFact) -> Result<(), FactoryAttemptError> {
    let owner = match fact.kind.as_str() {
        "now" | "day" | "flow" | "source-revision" | "source-change" | "receiving" => Some("central"),
...
fn nonempty(value: &str, field: &str) -> Result<(), FactoryAttemptError> {
    if value.trim().is_empty() { Err(invalid(&format!("{field} must be explicit"))) } else { Ok(()) }
}
...
pub fn execute_attempt_cli(args: &[String], stdin_override: Option<&str>) -> Result<String, FactoryAttemptError> {
    ...
    if positional.first().map(|value| value.as_str()) != Some("action") {
        if positional.first().map(|value| value.as_str()) == Some("read") {
            let path = positional.get(1).ok_or_else(|| invalid("missing state path"))?;
            let reading = read_attempts(Path::new(path))?;
            if args.iter().any(|arg| arg == "--json") { return Ok(serde_json::to_string_pretty(&reading)?); }
            let mut output = format!("{}\nRun: {} @ {}\nState revision: {}", reading.contract, reading.run_ref, reading.run_revision, reading.revision);
            for record in &reading.attempts {
                let leg = &reading.legs[&record.workflow_unit_ref];
                let historic = leg.execution_ref != execution_identity(record);
                let status = if historic { "historical".to_owned() } else { format!("{:?}", leg.status) };
                output.push_str(&format!("\n\nTask: {}\nAttempt: {} ({})\nAgent: {}\nSource: {} @ {}\nExecution: {}", record.task_ref, record.attempt_ref, status, record.disposition.participant.agent_ref, record.disposition.participant.source_ref, record.disposition.participant.source_revision, record.execution_ref.as_deref().unwrap_or("not dispatched")));
                if let Some(result) = &record.readable_return { output.push_str(&format!("\nReturn {}: {}", result.return_ref, result.summary)); }
                if has_uncertain_operation(record) { output.push_str("\nOwner outcome uncertain; reconciliation required."); }
                if !historic && leg.status != LegStatus::Returned { output.push_str("\nNot a completed Factory Return."); }
            }
            return Ok(output);
        }
    ...
    let input = if input_path != "-" { fs::read_to_string(input_path)? } else if let Some(input) = stdin_override { input.to_owned() } else {
    ...
    if args.iter().any(|arg| arg == "--json") { return Ok(serde_json::to_string_pretty(&receipt)?); }
    Ok(format!("{}\nRun: {}\nOperation: {}\nRevision: {} -> {}\n{}", receipt.contract, receipt.run_ref, receipt.operation, receipt.previous_revision, receipt.next_revision, receipt.standing))
}
