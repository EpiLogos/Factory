...
fn coherent_read(path: &Path, run: &RunRef) -> Result<(FactoryDevelopmentalState, FactoryAttemptReading), String> {
    ...
    for _ in 0..8 {
        ...
        if before == after { return Ok((after, reading)); }
    }
    Err("native state changed throughout task read; retry a fresh bounded reading".into())
}

fn attempt_view(state: &FactoryDevelopmentalState, reading: &FactoryAttemptReading, attempt: &FactoryAttemptRecord) -> Value {
    ...
    let historical = leg.and_then(|leg| leg.attempts.iter().find(|item| item.execution_ref == execution));
    let status = if current { leg.map(|leg| leg.status) } else { historical.map(|item| item.status) };
    ...
    let correlations = state.execution_correlations.iter().filter(|correlation|
        correlation.run_ref == reading.run_ref
            && attempt.execution_ref.as_ref() == Some(&correlation.execution_ref)
            && correlation.workflow_unit_ref == attempt.workflow_unit_ref).collect::<Vec<_>>();
    let model_observed = correlations.iter().any(|value| !value.model_usage.observations.is_empty());
    let material_observed = correlations.iter().any(|value| !value.material_usage.observations.is_empty());
    ...
    let unresolved = latest.values().filter(|receipt| matches!(receipt.phase,
        ...
        .collect::<Vec<_>>();
    let learning_refs = attempt.tracking.iter().filter(|fact| fact.kind == "regression-observation")
        .map(|fact| fact.subject_ref.clone()).chain(attempt.readable_return.iter()
            .flat_map(|returned| returned.regression_observation_refs.iter().cloned())).collect::<BTreeSet<_>>();
    json!({
...
pub fn read_task(path: &Path, run: &RunRef, task_ref: &str, limit: usize, cursor: Option<&TaskCursor>) -> Result<Value, String> {
    ...
    let mut attempts = reading.attempts.iter().filter(|attempt| attempt.task_ref == task_ref).collect::<Vec<_>>();
    attempts.sort_by(|left, right| left.attempt_ref.cmp(&right.attempt_ref));
    if attempts.is_empty() { return Err(format!("task {task_ref} has no retained attempt in {run}")); }
    let start = if let Some(cursor) = cursor {
        if cursor.revision != reading.revision { return Err("stale task cursor; restart at the current revision".into()); }
        attempts.iter().position(|attempt| attempt.attempt_ref == cursor.after_attempt_ref)
            .ok_or("task cursor does not belong to the addressed task")? + 1
    } else { 0 };
    let end = (start + limit).min(attempts.len());
    let next = if end < attempts.len() {
        Some(TaskCursor { revision:reading.revision, after_attempt_ref:attempts[end - 1].attempt_ref.clone() })
    } else { None };
    let views = attempts[start..end].iter().map(|attempt| attempt_view(&state, &reading, attempt)).collect::<Vec<_>>();
    Ok(json!({"contract":TASK_READING,"projectRef":state.build.project().reference(),"runRef":run,
...
pub fn read_return(path: &Path, run: &RunRef, attempt_ref: &str) -> Result<Value, String> {
    let (state, reading) = coherent_read(path, run)?;
    let attempt = reading.attempts.iter().find(|attempt| attempt.attempt_ref == attempt_ref)
        .ok_or_else(|| format!("attempt {attempt_ref} is not retained in {run}"))?;
    Ok(json!({"contract":RETURN_READING,"projectRef":state.build.project().reference(),"runRef":run,
...
fn strings(value: &Value) -> String {
    value.as_array().map(|items| items.iter().filter_map(Value::as_str).collect::<Vec<_>>().join(", "))
        .filter(|value| !value.is_empty()).unwrap_or_else(|| "none retained".into())
}
fn text(value: &Value) -> &str { value.as_str().unwrap_or("not available") }
...
pub fn render_reading(value: &Value) -> String {
    ...
    let views = if let Some(views) = value["attempts"].as_array() { views.iter().collect::<Vec<_>>() }
        else { vec![&value["attempt"]] };
    for view in views {
        ...
        if returned.is_null() { lines.push("Return: not recorded (provider acknowledgement is not task completion)".into()); }
        else {
        ...
        for fact in record["tracking"].as_array().into_iter().flatten() {
            lines.push(format!("Correlation: {} / {} — {} @ {}",text(&fact["kind"]),text(&fact["ownerRef"]),text(&fact["subjectRef"]),text(&fact["sourceRevision"])));
        }
        for receipt in view["unresolvedOwnerOperations"].as_array().into_iter().flatten() {
            lines.push(format!("Unresolved owner effect: {} / {}",text(&receipt["ownerRef"]),text(&receipt["operationRef"])));
        }
    }
    if !value["nextCursor"].is_null() { lines.push(format!("Next cursor: {}",value["nextCursor"])); }
    lines.join("\n")
}

pub fn execute_cli(args: &[String]) -> Result<String, String> {
    if args.len() < 4 { return Err("Usage: factory attempt task|return <state> <run-ref> <task-ref|attempt-ref> [--json] [--limit N] [--cursor JSON]".into()); }
    let run: RunRef = args[2].parse().map_err(|error| format!("invalid Run: {error}"))?;
    let mut limit = 50;
    let mut cursor = None;
    let mut json = false;
    let mut index = 4;
    while index < args.len() {
        match args[index].as_str() {
            "--json" => json = true,
            "--limit" => { index += 1; limit = args.get(index).ok_or("missing page limit")?.parse::<usize>().map_err(|error| error.to_string())?; }
            "--cursor" => { index += 1; cursor = Some(serde_json::from_str::<TaskCursor>(args.get(index).ok_or("missing cursor")?).map_err(|error| error.to_string())?); }
    ...
    let reading = match args[0].as_str() {
        ...
        "return" if cursor.is_none() && limit == 50 => read_return(Path::new(&args[1]), &run, &args[3])?,
        _ => return Err("unsupported read operation or pagination on a single Return".into()),
    };
    if json { serde_json::to_string_pretty(&reading).map_err(|error| error.to_string()) }
        else { Ok(render_reading(&reading)) }
}
