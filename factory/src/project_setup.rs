...
fn reject_symlink(path: &Path) -> Result<(), String> {
    match fs::symlink_metadata(path) {
        Ok(meta) if meta.file_type().is_symlink() => Err(format!(
            "Factory placement refuses symlink {}",
            path.display()
        )),
        Ok(_) => Ok(()),
...
fn reading(root: &Path, placement: &Placement, status: &str) -> Result<Value, String> {
    if placement.schema != SCHEMA
        || placement.state_file != STATE
        || project_ref_for_key(&placement.project_key)
            .map_err(error)?
            .to_string()
            != placement.factory_project_ref
    {
    ...
    if provider.state().build.project().reference().to_string() != placement.factory_project_ref {
        return Err(
            "Factory placement and native Project identity disagree; existing state preserved"
                .into(),
        );
    }
    Ok(
        json!({"contract":"factory.project-location/v1", "status":status,
...
pub fn locate(root: &Path) -> Result<Value, String> {
    ...
    let placement: Placement =
        serde_json::from_slice(&fs::read(dir.join("project.json")).map_err(error)?)
            .map_err(error)?;
...
pub fn setup_central(root: &Path, central_ref: &str, source: &Path) -> Result<Value, String> {
    ...
    if root.join(".factory/project.json").exists() {
        ...
        key = existing["projectKey"]
            .as_str()
            .ok_or("Factory placement omitted its key")?
            .into();
...
fn setup_bound(
    ...
    let placement = Placement {
        ...
        factory_project_ref: project_ref.to_string(),
        state_file: STATE.into(),
    };
    // Validate the source before making any filesystem changes.
    let link = central_source
        .map(|source| -> Result<_, String> {
            ...
            {
                return Err(
                    "Factory placement requires this Project's own Central manifest".into(),
                );
            }
            let request = FactoryCentralProjectLinkRequest {
                contract: FACTORY_CENTRAL_PROJECT_LINK_REQUEST.into(),
                factory_project_ref: project_ref.clone(),
                central_project_ref: central_ref.into(),
    ...
    if placement_path.exists() {
        let existing: Placement =
            serde_json::from_slice(&fs::read(&placement_path).map_err(error)?).map_err(error)?;
        if existing != placement {
            return Err(
                "Factory Project is already bound to a different native scope; no state changed"
                    .into(),
            );
        }
    }
