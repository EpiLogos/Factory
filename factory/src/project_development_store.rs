...
fn validate_ledger(
    ...
    for observation in &ledger.observations {
        if &observation.run_ref != run_ref
            || observation.observation_ref.trim().is_empty()
            || !observations.insert(&observation.observation_ref)
        {
            return Err(ProjectDevelopmentStoreError::Native(
                "development observation has duplicate or foreign Run identity".into(),
            ));
        }
    }
    Ok(())
...
impl ProjectDevelopmentStore for FileProjectDevelopmentStore {
    fn save(&self, ledger: &ProjectDevelopmentLedger) -> Result<(), ProjectDevelopmentStoreError> {
        ...
        let lock_path = self
            .root
            .join(format!(".{}.lock", ledger.run_ref.as_ref().id()));
        ...
        if fs::symlink_metadata(&path).is_ok_and(|metadata| metadata.file_type().is_symlink()) {
            return Err(ProjectDevelopmentStoreError::Native(
                "development ledger must not replace a symlink target".into(),
            ));
        }
