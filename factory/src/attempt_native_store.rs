...
impl FactoryRunAttempts {
    ...
    pub(crate) fn validate_identity(&self, run_ref: &RunRef) -> Result<(), FactoryAttemptError> {
        if self.schema != FACTORY_RUN_ATTEMPTS || self.snapshot.run_ref() != run_ref {
            return Err(invalid("native attempt metadata does not belong to its canonical Run"));
        }
        Ok(())
    }
}
...
impl FileAttemptStore {
    ...
    pub fn initialize(path: impl Into<PathBuf>, seed: FactoryAttemptSeed) -> Result<Self, FactoryAttemptError> {
        let path = path.into();
        if path.exists() { return Err(FactoryAttemptError::AlreadyExists(path)); }
        ...
        let build = FactoryBuildState::new(Project::new(engine.run().project_ref().clone()), engine.run().clone())
        ...
        let view = StoredAttemptState {
            schema: FACTORY_ATTEMPT_STATE.into(), revision: native.build.revision().get(),
            run: engine.run().clone(), workflow_source: seed.workflow_source,
            snapshot: engine.snapshot(), attempts: BTreeMap::new(), action_receipts: BTreeMap::new(),
        };
        validate_state(&view)?;
        native.attempt_states.insert(run_ref.clone(), FactoryRunAttempts::from_view(&view));
    ...
    pub fn attach(path: impl Into<PathBuf>, run_ref: RunRef, source_ref: &str) -> Result<Self, FactoryAttemptError> {
        let path = path.into();
        transact_developmental_state(&path, |native| {
            (|| -> Result<(), FactoryAttemptError> {
                ...
                let run = native.build.run(&run_ref).cloned()
                    .ok_or_else(|| invalid("canonical Run is not present in the native provider"))?;
                let source = native.workflow_sources.iter()
                    .find(|source| source.source.reference.to_string() == source_ref)
                    .cloned().ok_or_else(|| invalid("workflow source is not admitted in the native provider"))?;
                let engine = ExecutableOrchestration::new(compile_workflow(source.clone())?, run.clone())?;
                native.build.replace_attempt_run(run.revision(), engine.run().clone())
                    .map_err(|error| invalid(&error.to_string()))?;
                let view = StoredAttemptState {
                    schema: FACTORY_ATTEMPT_STATE.into(), revision: native.build.revision().get(),
                    run: engine.run().clone(), workflow_source: source, snapshot: engine.snapshot(),
                    attempts: BTreeMap::new(), action_receipts: BTreeMap::new(),
                };
                validate_state(&view)?;
                native.attempt_states.insert(run_ref.clone(), FactoryRunAttempts::from_view(&view));
                Ok(())
    ...
    pub fn open(path: impl Into<PathBuf>) -> Result<Self, FactoryAttemptError> {
        ...
        let run_ref = native.attempt_states.keys().next().expect("one checked entry").clone();
        view_for(&native, &run_ref)?;
        Ok(Self { path, run_ref })
    }

    pub fn open_run(path: impl Into<PathBuf>, run_ref: RunRef) -> Result<Self, FactoryAttemptError> {
        let path = path.into();
        let native = read_developmental_state(&path).map_err(native_error)?;
        view_for(&native, &run_ref)?;
        Ok(Self { path, run_ref })
    ...
    pub fn apply(&mut self, request: FactoryAttemptActionRequest) -> Result<FactoryAttemptActionReceipt, FactoryAttemptError> {
        validate_action_request(&request)?;
        if request.run_ref != self.run_ref {
            return Err(FactoryAttemptError::RunMismatch { addressed: request.run_ref.to_string(), stored: self.run_ref.to_string() });
        }
        let request_digest = blake3::hash(&serde_json::to_vec(&request)?).to_hex().to_string();
...
fn view_for(native: &FactoryDevelopmentalState, run_ref: &RunRef) -> Result<StoredAttemptState, FactoryAttemptError> {
    let retained = native.attempt_states.get(run_ref).ok_or_else(|| invalid("Run has no native attempt field; attach its admitted source first"))?;
    retained.validate_identity(run_ref)?;
    let run = native.build.run(run_ref).cloned().ok_or_else(|| invalid("attempt field has no canonical Run"))?;
    let view = StoredAttemptState {
        schema: FACTORY_ATTEMPT_STATE.into(), revision: native.build.revision().get(), run,
        workflow_source: retained.workflow_source.clone(), snapshot: retained.snapshot.clone(),
        attempts: retained.attempts.clone(), action_receipts: retained.action_receipts.clone(),
    };
    validate_state(&view)?;
    Ok(view)
}
...
fn source_is_current(native: &FactoryDevelopmentalState, view: &StoredAttemptState) -> bool {
    native.workflow_sources.iter().any(|source| source.source == view.workflow_source.source && source.workflow_key == view.workflow_source.workflow_key)
}

fn requires_current_source(operation: &FactoryAttemptOperation) -> bool {
    matches!(operation,
        FactoryAttemptOperation::StartSerial { .. } | FactoryAttemptOperation::StartFork { .. }
        | FactoryAttemptOperation::BindDispatch { .. } | FactoryAttemptOperation::Retry { .. }
        | FactoryAttemptOperation::ReturnArtifact { .. } | FactoryAttemptOperation::IncorporateLateResult { .. })
}

fn invalid(message: &str) -> FactoryAttemptError { FactoryAttemptError::InvalidOperation(message.into()) }
fn store_error(error: FactoryAttemptError) -> ProjectDevelopmentStoreError { ProjectDevelopmentStoreError::Native(error.to_string()) }
fn native_error(error: ProjectDevelopmentStoreError) -> FactoryAttemptError { invalid(&error.to_string()) }
