...
- An installed `telemetry stats --day` once exited zero with whole-provider data. Verify the returned Day window, policy revision, and source coverage before using statistics; a success exit is not a bounded reading. Native regression: `factory/src/telemetry_cli.rs` `public_cli_day_stats_exclude_untimed_native_records`.
